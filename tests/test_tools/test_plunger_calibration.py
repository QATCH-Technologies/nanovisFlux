...
def test_sorts_out_of_order_input():
    cal = PlungerCalibration.from_pairs(
        aspirate=[(1000, 100.0), (2000, 0.0), (0, 220.0)],
        dispense=[(1000, 0.0), (0, 95.0)],
    )
    assert [p.volume_ul for p in cal.aspirate_points] == [0.0, 100.0, 220.0]


# -- interpolation ----------------------------------------------------------
# Points run HIGH microsteps -> LOW volume down to LOW microsteps -> HIGH
# volume, matching the real hardware convention (see PlungerModel.
# volume_to_microsteps's subtraction and the real calibration data in
# configs/tools/pipettes/.../calibrations/*.yaml, e.g. bottom_microsteps
# 15000 == 0uL, decreasing as volume increases) -- not the ascending
# direction these fixtures used before.
@pytest.fixture
def cal() -> PlungerCalibration:
    return PlungerCalibration.from_pairs(
        aspirate=[(2000, 0.0), (1000, 100.0), (0, 220.0)],
        dispense=[(2000, 0.0), (1000, 95.0), (0, 200.0)],
    )
...
def test_exact_match_at_measured_points(cal):
    ...
    assert cal.microsteps_for_volume(220.0, aspirating=True) == 0
    assert cal.volume_for_microsteps(1000, aspirating=True) == pytest.approx(100.0)


def test_interpolates_between_points(cal):
    # Halfway between (2000, 0uL) and (1000, 100uL) -> 1500 microsteps.
    assert cal.microsteps_for_volume(50.0, aspirating=True) == 1500
    # Dispense curve is deliberately different: (2000,0uL)->(1000,95uL).
    assert cal.microsteps_for_volume(50.0, aspirating=False) == round(2000 - 50.0 / 95.0 * 1000)


def test_extrapolates_using_nearest_segment_slope(cal):
    # Last aspirate segment: (1000, 100uL) -> (0, 220uL), slope -1000/120 usteps/uL.
    expected = round(0 + (300.0 - 220.0) * (0 - 1000) / (220.0 - 100.0))
    assert cal.microsteps_for_volume(300.0, aspirating=True) == expected
    # Below the lowest point extrapolates the first segment instead of clamping to 2000.
    expected_low = round(2000 + (-10.0 - 0.0) * (1000 - 2000) / (100.0 - 0.0))
...
def _pipette_with(plunger, tip_calibrations=None) -> tuple:
    robot = MagicMock()
    pip = Pipette(
        name="p300", plunger=plunger, max_volume_ul=300, tip_calibrations=tip_calibrations
    )
...
def test_pipette_falls_back_to_linear_model_with_no_calibration():
    ...
    pip.aspirate(10.0)
    assert _last_target(robot) == {AxisId.B: 500}  # 1000 - 10*50


def test_pipette_falls_back_when_tip_has_no_calibration_entry():
    tip = TipGeometry(name="unknown_tip", length_mm=50.0)
    pip, robot = _pipette_with(
        PlungerModel(microsteps_per_ul=50.0, bottom_microsteps=0), tip_calibrations={}
    )
    pip.current_tip = tip
    pip.aspirate(10.0)
...
def test_pipette_uses_calibration_for_current_tip():
    ...
    cal = PlungerCalibration.from_pairs(
        aspirate=[(2000, 0.0), (0, 100.0)],
        dispense=[(1800, 0.0), (0, 100.0)],  # deliberately different from aspirate
    )
    pip, robot = _pipette_with(
        PlungerModel(microsteps_per_ul=50.0), tip_calibrations={"p300_tip": cal}
    )
    pip.current_tip = tip

    pip.aspirate(50.0)
    assert _last_target(robot) == {AxisId.B: 1000}  # aspirate curve: 2000 - 50/100*2000

    pip.dispense(25.0)  # current_volume_ul: 50 -> 25
    assert _last_target(robot) == {
        AxisId.B: 1350
    }  # dispense curve: 1800 - 25/100*1800, not aspirate's 1500
...
def test_pipette_blow_out_uses_dispense_direction():
    ...
    cal = PlungerCalibration.from_pairs(
        aspirate=[(2000, 0.0), (0, 100.0)],
        dispense=[(1800, 10.0), (0, 100.0)],  # dispense curve doesn't pass through (x, 0)
    )
    pip, robot = _pipette_with(
        PlungerModel(microsteps_per_ul=50.0), tip_calibrations={"p300_tip": cal}
    )
