...
from PyQt5.QtWidgets import (QDialog, QVBoxLayout, QFormLayout, QComboBox, QLineEdit,
                             QDoubleSpinBox, QSpinBox, QDialogButtonBox, QStackedWidget,
                             QWidget, QMessageBox)

from ..geometry.coordinates import DeckPoint
from ..deck import WellPlateDefinition, ReservoirDefinition, TipRackDefinition, WellShape, BottomShape
...
class LabwareDialog(QDialog):
    def __init__(self, slot, parent=None):
        ...
        self.footprint_x = QDoubleSpinBox(); self.footprint_x.setRange(1, 1000); self.footprint_x.setValue(sw)
        self.footprint_y = QDoubleSpinBox(); self.footprint_y.setRange(1, 1000); self.footprint_y.setValue(sh)
        self.height = QDoubleSpinBox(); self.height.setRange(0, 500); self.height.setValue(14.4)
        self.rows = QSpinBox(); self.rows.setRange(1, 64); self.rows.setValue(8)
        self.cols = QSpinBox(); self.cols.setRange(1, 64); self.cols.setValue(12)
        self.row_spacing = QDoubleSpinBox(); self.row_spacing.setRange(0.1, 500); self.row_spacing.setValue(9.0)
        self.col_spacing = QDoubleSpinBox(); self.col_spacing.setRange(0.1, 500); self.col_spacing.setValue(9.0)
        self.origin_x = QDoubleSpinBox(); self.origin_x.setRange(-500, 500); self.origin_x.setValue(14.4)
        self.origin_y = QDoubleSpinBox(); self.origin_y.setRange(-500, 500); self.origin_y.setValue(11.2)
        self.origin_z = QDoubleSpinBox(); self.origin_z.setRange(-500, 500); self.origin_z.setValue(14.4)
        ...
        self.well_diameter = QDoubleSpinBox(); self.well_diameter.setRange(0, 200); self.well_diameter.setValue(6.9)
        self.well_width = QDoubleSpinBox(); self.well_width.setRange(0, 200)
        self.well_length = QDoubleSpinBox(); self.well_length.setRange(0, 200)
        self.well_depth = QDoubleSpinBox(); self.well_depth.setRange(0, 200); self.well_depth.setValue(10.9)
        self.well_bottom = QComboBox()
        self.well_bottom.addItems(["flat", "round", "v"])
        self.well_bottom.setCurrentText("round")
        self.bottom_clearance = QDoubleSpinBox(); self.bottom_clearance.setRange(0, 100); self.bottom_clearance.setValue(1.5)
        self.well_volume = QDoubleSpinBox(); self.well_volume.setRange(0, 100000); self.well_volume.setValue(360)
        ...
        self.tip_length = QDoubleSpinBox(); self.tip_length.setRange(0, 200); self.tip_length.setValue(51.7)
        self.tip_volume = QDoubleSpinBox(); self.tip_volume.setRange(0, 10000); self.tip_volume.setValue(300)
    ...
    def _on_accept(self) -> None:
        ...
        common_kwargs = dict(
            ...
            row_spacing_mm=self.row_spacing.value(), col_spacing_mm=self.col_spacing.value(),
            grid_offset=DeckPoint(self.origin_x.value(), self.origin_y.value(), self.origin_z.value()),
        )
        try:
            if kind == "tip rack":
                self.definition = TipRackDefinition(
                    **common_kwargs, tip_volume_ul=self.tip_volume.value(),
                    tip_length_mm=self.tip_length.value())
            else:
                cls = ReservoirDefinition if kind == "reservoir" else WellPlateDefinition
                self.definition = cls(
                    **common_kwargs, well_volume_ul=self.well_volume.value(),
                    well_shape=WellShape(self.well_shape.currentText()),
                    well_diameter_mm=self.well_diameter.value(),
                    well_width_mm=self.well_width.value(), well_length_mm=self.well_length.value(),
